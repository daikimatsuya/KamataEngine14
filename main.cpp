...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Sphere pointSphere{ point,0.01 };
	Sphere closestPointSphre{ closestPoint,0.01f };
	...
	while (Novice::ProcessMessage() == 0) {
		...
		Vector3 project = ltpc_.Project(vf_.Subtract(point, segment.origin), segment.diff);
		ImGui::Begin("Window");
		ImGui::DragFloat3("Project", &project.x,0.0f);
		ImGui::End();
		closestPointSphre.center = ltpc_.ClosestPoint(point, segment);
		///
		/// ↑更新処理ここまで
		///
		...
		dsag_.DrwaSphere(pointSphere, worldViewProjectionMatrix, viewPortMatrix, RED);
		dsag_.DrwaSphere(closestPointSphre, worldViewProjectionMatrix, viewPortMatrix, BLACK);
